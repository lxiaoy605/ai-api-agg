# AI 模型 API 聚合平台 — 完整技术执行方案

> **版本：** 最终版（整合 v5 技术报告 + v6 模型厂商调研 + DESIGN.md 设计系统）
> **日期：** 2026-05-27
> **用途：** 后续拆分子任务、交付 hermes / claude-code 执行的权威基准文档
...
| # | 章节 | 定位 | 篇幅 |
|---|------|------|------|
| 2 | 模型厂商对接方案 | 基础层 - 8 家国内厂商对接评估 | 厂商全景 + 接入方案 |
| 3 | 海外账号注册分析 | 战略层 - 如何获取 GPU 之外的海外 AI 资源 | 深度分析 + 行动方案 |
| 5 | 蓝绿部署 + Key 轮换 + 回滚 | 运维层 - 生产级零停机基础设施 | 完整工程方案 |
| 6 | Stripe + USDT 自动支付方案 | 商业层 - 双轨自动支付系统 | 完整工程方案 |
| 7 | RESTful 管理 API + 配置即代码 + 灾难恢复 | 管控层 - 可编程基础设施 | 完整工程方案 |
| 8 | AI 自动运维 | 智能层 - 自愈型运维体系 | 深度分析 + 工程方案 |
...
| 核心中间件 | OneAPI（MIT 开源，github.com/songquanpeng/one-api） |
| 技术栈 | Python/Go 后端、轻量前端、Docker Compose 单机部署 |

### 1.3 三层架构概述
```
用户端前端（Next.js + Tailwind + shadcn/ui）
    ↕ REST API
OneAPI 后台网关（渠道转发 / 限流 / 计量）
    ↕ API
国内模型源（DeepSeek / 智谱Z.ai / 小米MiMo / ...）
```
...
- **海外用户：** 完全可行，邮箱即可
- **支付方式：** 国际信用卡
- **API 认证：** Bearer Token，OpenAI 兼容格式
- **免费额度：** 新用户赠送 500 万 tokens
- **速率限制：** 标准 RPM/TPM 限制
- **模型列表：** deepseek-v4-flash（非思考）、deepseek-v4-pro（思考模式）
- **备注：** 2026/07/24 后 deepseek-chat / deepseek-reasoner 旧名废弃
- **结论：** 🥇 对海外开发者最友好，MVP 首选

#### 2.1.2 智谱 Z.ai ⭐⭐⭐⭐ — 首发

- **国内站：** open.bigmodel.cn（需中国手机号 +86）
- **国际站：** z.ai（支持邮箱/Google 注册，无需手机号）
- **关键发现：** Z.ai 是独立域名的国际化平台，非国内站的简单翻译版，有独立账号体系和计费系统
- **实名认证：** 国际站不需要
- **支付：** 国际站 Stripe 美元支付，Coding Plan 订阅制（$18-$49/月）
- **API 认证：** Bearer Token，OpenAI 兼容 / Anthropic 兼容
- **Base URL：** `https://api.z.ai/api/paas/v4`（通用）或 `https://api.z.ai/api/coding/paas/v4`（Coding Plan）
- **免费额度：** GLM-4.7-Flash 免费
- **定价（USD）：** GLM-5.1 $1.40/$4.40 per 1M in/out, GLM-5 $1.00/$3.20
- **结论：** 🥇 国际站对海外用户友好，Stripe 支付，与平台支付体系一致

#### 2.1.3 小米 MiMo ⭐⭐⭐⭐ — 首发

- **官网：** platform.xiaomimimo.com
- **注册方式：** 邮箱注册，海外用户无需实名认证
- **来源确认：** "海外用户无需实名即可直接充值"（中关村在线/IT之家/网易报道）
- **支付：** 国际信用卡 / Apple Pay / Google Pay
- **定价：** Token Plan 订阅包月（$6/月起）或按量计费
- **Base URL：** `https://api.xiaomimimo.com/v1`
- **API 认证：** Bearer Token，OpenAI 兼容
- **模型：** MiMo-V2.5-Pro / V2-Pro / V2-Omni / V2-Flash / V2-TTS
- **速率限制：** RPM 100 / TPM 10M
- **结论：** 🥇 海外友好，美元定价，梦遥已有账号

#### 2.1.4 阿里 DashScope（百炼）⭐⭐⭐ — 第 2 批

- **国际站：** 阿里云国际站可用
- **注册要求：** 需当地手机号 + 信用卡
- **支付：** 国际信用卡
- **API：** OpenAI 兼容格式
- **结论：** 🥈 可接入但门槛较高，第 2 阶段处理
...
- **支付：** 以 CNY 为主
- **企业/个人认证要求较高**
- **结论：** 🥈 模型能力强但接入门槛高，第 2 阶段处理

#### 2.1.6 零一万物 Yi ⭐ — 需中转

- **注册：** 需中国手机号
- **支付：** 仅微信/支付宝，不支持外币
- **替代方案：** 通过 OpenRouter 中转获取
- **结论：** 🥉 不直接接入，走 OpenRouter
...
| 优先级 | 厂商 | 理由 |
|:---:|------|------|
| 🥇 首发 | DeepSeek + 智谱Z.ai + 小米MiMo | 三家海外友好，可直接注册/充值 |
| 🥈 第2批 | 阿里DashScope + 腾讯混元 | 可接但有注册/支付门槛 |
| 🥉 中转 | 零一万物Yi | 走 OpenRouter |
| ❌ 跳过 | 字节豆包、月暗Kimi | 海外无法注册 |

### 2.3 账号池管理策略

**方案：** OneAPI 多账号 Key 池 + 权重轮询 + 健康检查自动下架

1. **多账号轮询：** 同一厂商配置多个 API Key（不同账号），OneAPI 按权重自动分发
2. **健康检查：** 每 30 秒探测各渠道可用性，连续失败 3 次自动降权或下架
3. **Key 安全存储：** 加密环境变量（小团队）/ 未来可升级 Vault
4. **Key 轮换：** 90 天自动轮换 + 预热验证 + 双 Key 并存过渡 + 滞后删除旧 Key
5. **厂商并发限制应对：** 多账号分散请求 + 令牌桶限流 + 429 自动退避重试

### 2.4 多邮箱申请方案

#### 2.4.1 域名邮箱策略

使用自有域名 + 邮件转发服务来批量注册和管理厂商账号：

| 组件 | 方案 | 费用 |
|------|------|------|
| 域名 | 任意通用域名（如 example.dev） | ~$10/年（Namecheap / Cloudflare / Porkbun） |
| 邮件转发 | Cloudflare Email Routing（免费） | $0 |
| 备用 | Zoho Mail 免费版（5 个邮箱） | $0 |
| 别名上限 | Cloudflare：无限 catch-all；每个厂商子域名均可 | $∞ |

**工作流：**
1. 注册域名后配置通配邮箱 `*@yourdomain.dev`
2. 按厂商分配子邮箱：`deepseek@yourdomain.dev`、`zhipu@yourdomain.dev`、`mimo@yourdomain.dev`
3. 所有邮件统一转发至主邮箱（或 Telegram Bot 通知）
4. 注册时用对应子邮箱，方便后期按厂商管理账号

#### 2.4.2 各厂商账号规划

| 厂商 | 推荐账号数 | 理由 | 免费额度 |
|------|:---------:|------|---------|
| DeepSeek | 2-3 个 | 免费 500 万 tokens/账号，轮询复用率高 | 500 万 tokens |
| 智谱 Z.ai | 1-2 个 | Coding Plan $18-$49/月，按需付费 | GLM-4.7-Flash 免费 |
| 小米 MiMo | 1-2 个 | Token Plan $6/月起 | 注册赠送额度 |
| 阿里 DashScope | 1 个 | 实名制限制，仅国内用户可用 | 少量赠送 |
| 腾讯混元 | 1 个 | 实名制限制 | 少量赠送 |

**账号冗余逻辑：** 主力厂商（DeepSeek、智谱、MiMo）配置 2-3 个账号作为 OneAPI 渠道池，通过权重轮询将请求分散到多个 API Key 上。单个账号被限速或额度耗尽时自动切换至下一个可用账号。

#### 2.4.3 邮箱成本对比

| 方案 | 年费 | 别名数量 | 适用场景 |
|------|:---:|:--------:|---------|
| Cloudflare Email Routing | $0 | 无限 | 推荐，配合自有域名 |
| Zoho Mail 免费版 | $0 | 5 个 | 备用，无需域名 |
| iCloud+ 自定义域名 | $0.99/月 | 5 个 | 苹果生态用户 |
| ProtonMail 付费 | $3.99/月 | 无限 | 高隐私需求 |
| 域名 + catch-all | ~$10/年 | 无限 | 最推荐，年成本最低 |

**推荐策略：** 注册一个 ~$10/年的域名 + Cloudflare Email Routing（免费），即可无限创建邮箱别名。总成本仅为域名年费。
...
| 厂商 | 余额查询方式 | API 端点 | 认证 |
|------|-------------|---------|------|
| DeepSeek | 官方 API | `GET /user/balance` | Bearer Token |
| 智谱 Z.ai | 国际站 Dashboard / API | `GET /api/paas/v4/profile` | Bearer Token |
| 小米 MiMo | Dashboard 手动查看 | 暂未公开余额 API | 无 |
| 阿里 DashScope | API | `GET /api/v1/balance` | API Key |
| 腾讯混元 | Dashboard / 账单 API | 需登录控制台 | Cookie/Session |

> **注意：** 小米 MiMo 未开放余额查询 API，需通过定时爬取 Dashboard 或人工查看。建议 MiMo 账号手动预充大额（如 $50），降低检查频率。

#### 2.5.2 统一余额监控脚本设计

**余额监控脚本（balance_monitor.py）：**
- 定时（cron: */30 * * * *）扫描所有厂商 Key 余额
- 每个厂商实现独立的 check_balance() 函数
- 余额低于阈值时触发 Telegram + 邮件告警
- 记录到审计日志供后续分析
...
| 级别 | 阈值（USD） | 动作 | 通知方式 |
|:----:|:----------:|------|---------|
| 紧急 | < $2 | 立即告警 + 从备用渠道自动切流 | Telegram + 邮件 |
| 预警 | < $5 | 提醒充值 | Telegram |
| 正常 | < $10 | 更新状态到仪表盘 | 无通知 |
| 充裕 | >= $10 | 无操作 | - |
...
1. 收到余额预警通知
2. 登录厂商 Dashboard
3. 使用 Stripe / 国际信用卡完成充值
4. 更新 OneAPI 渠道配置中的新 Key（如需要）
5. 记录充值操作到审计日志

**自动充值（V2 目标）：**
1. 余额监控脚本检测到低于预警阈值
2. 调用厂商预充值 API（DeepSeek/智谱支持自动充值）
3. 使用绑定的 Stripe 卡完成支付
4. 验证充值成功后恢复 Key 状态
5. 通过 Telegram 通知充值完成

#### 2.5.5 欠费预防策略

1. **提前 48 小时预警：** cron 每天检查一次余额，低于阈值即通知
2. **备用渠道自动切换：** 主账号余额不足时，OneAPI 自动路由到备用账号
3. **跨厂商冗余：** Key 用尽时自动切到另一家厂商的同能力模型（如 DeepSeek -> 智谱）
4. **账户充值隔离：** 每个厂商预留至少 $10 应急余额，不作为日常消费池
5. **历史消耗预测：** 按近 7 天日均消耗 x 30 天估算下次充值时间和金额
...
> 本章内容已整合至第 2 章模型厂商对接方案的 2.4（多邮箱申请方案）和 2.5（多账号余额监控）小节。
...
采用 Google Stitch 的 **DESIGN.md** 标准（github.com/VoltAgent/awesome-design-md），
选定 **Together AI** 的设计语言作为参考基准——Together AI 本身就是 AI 基础设施平台，
其设计风格天然匹配面向开发者的 API 聚合平台。

#### 4.1.1 设计系统速查

| 元素 | 值 | 用途 |
|------|------|------|
| 主背景 | `#000000` / `#ffffff` | 暗色英雄区 / 亮色内容区 |
| 主文字 | `#000000` / `#ffffff` | 高对比度可读性 |
| 辅文字 | `#959494` | 说明性文本 |
| 分割线 | `#959494` | 细线分割 |
| 强调橙 | `#fc4c02` | 主 CTA 按钮、链接 |
| 强调品红 | `#ef2cc1` | 次要强调、图标 |
| 强调薰衣草 | `#bdbbff` | 装饰渐变元素 |
| 深色表面 | `#010120` / `#313641` | 暗色卡片、代码区 |
| 成功绿 | `#c8f6f9` | 成功状态 |
...
| 层级 | 字族 | 用途 |
|------|------|------|
| 展示标题 | Custom Display Sans (可替代: Inter Display) | Hero 区大标题 |
| 标题 | Inter / SF Pro | 页面标题、卡片标题 |
| 正文 | Inter / SF Pro Text | 正文、表格、表单 |
| 代码 | JetBrains Mono / Fira Code | API 示例、代码块 |
| 技术标签 | Uppercase Mono | 模型名称标签、状态徽标 |

#### 4.1.3 设计原则
- **暗色/亮色交替：** 英雄区近黑底 + 三色渐变，文档/定价区纯白底
- **无装饰性元素：** 只靠字体、代码和微妙的渐变发光
- **圆角 12px 容器：** 统一的卡片/面板圆角
- **1px 半透明白色细线：** 暗色区边框
- **API 优先的排版：** 代码块与文字并重，终端风格点缀

### 4.2 技术栈

| 模块 | 选择 | 理由 |
|------|------|------|
| 框架 | Next.js 14+ (App Router) | React 生态，SEO 友好，Vercel 部署 |
| 样式 | Tailwind CSS 4 | 原子化 CSS，快速迭代 |
| 组件库 | shadcn/ui | 基于 Radix UI，现代简洁，可定制 |
| 图标 | Lucide React | 开源，齐全 |
| API 通信 | React Query (TanStack Query) | 缓存/重试/乐观更新 |
| 代码高亮 | Shiki | 终端风格 API 示例 |

### 4.3 核心页面

#### 4.3.1 首页（Landing）
- 暗色 Hero 区：三色渐变装饰 + 平台名称 + "One API Key, All Chinese Models"
- 模型目录区：亮色底，列出可用模型 + 价格对比
- 快速开始：暗色代码区，`curl` 示例一键复制
...
#### 4.3.3 Dashboard
- 顶部 KPI 卡片（今日请求量/成功率/延迟/花费）
- 用量趋势折线图（按模型分色）
- API Key 管理区（生成/复制/重置）

#### 4.3.4 API Key 管理
- 一键生成 Key
- Key 列表（部分脱敏显示）+ 复制/重置/删除
- 用量限制设置

#### 4.3.5 余额充值
- Stripe Checkout 集成（美元）
- USDT 地址展示 + 手动确认（MVP）
- 充值记录列表

#### 4.3.6 Developer Docs
- API 端点文档
- curl / Python / JavaScript 代码示例
- 模型命名规范 + 定价表

### 4.4 前端部署
- Vercel 免费层（个人项目完全够用）
- 或与后端同机 Nginx 反代（零额外成本）
- 环境变量注入 OneAPI 后端地址 + Stripe Publishable Key
...
## 7. RESTful 管理 API + 配置即代码 + 灾难恢复

### 7.1 RESTful 管理 API（OpenAPI 规范）

#### 7.1.1 API 规范文档（OpenAPI 3.0）
...
### 7.2 配置即代码（Infrastructure as Code）
...
| 周次 | 里程碑 | 具体任务 |
|:--:|------|------|
| **W1** | 🟢 基础设施 | OneAPI Docker 部署 → DeepSeek API 接入验证 → 智谱Z.ai 注册+接入 → 小米MiMo 接入 → 三渠道权重轮询测试 → 故障切换测试 |
| **W2** | 🔵 上线部署 | 买 Hetzner CX22 + 域名 → Docker Compose 一键部署 → SSL/HTTPS/Nginx 配置 → Stripe 最小集成 → 用户注册/登录 → API Key 自助管理 |
| **W3** | 🟡 前端完善 | Dashboard 仪表盘 → 用量图表 → 模型目录 → Developer Docs 页面 → USDT 充值页面 |
| **W4** | 🟣 验证上线 | 端到端测试 → 监控告警配置 → 备份脚本部署 → 邀请 2-3 个种子用户试 → 收集反馈迭代 |

### 9.2 子任务拆分指南

以上每个周次的任务均可进一步拆分为独立子任务，交付 hermes 或 claude-code 执行：

- **W1 子任务示例：** Docker Compose 编写、OneAPI 配置、Nginx 配置、DeepSeek 渠道配置、健康检查脚本
- **W2 子任务示例：** VPS 购买+SSH 配置、域名购买+DNS 配置、Stripe Webhook 处理器、用户注册 API、API Key 管理 CRUD
- **W3 子任务示例：** Dashboard 前端页面、用量图表组件、模型目录页面、Developer Docs 页面、USDT 地址展示组件
- **W4 子任务示例：** 端到端测试脚本、Prometheus 配置、备份脚本、种子用户邀请流程
...
1. 代码可运行，无明显 bug
2. 功能与本文档描述一致
3. 安全基线达标（HTTPS 强制、API Key 加密、速率限制）
4. 文档/注释完整
...
| 项目 | 金额 |
|------|------|
| Hetzner CX22（首月） | $4.35 |
| 域名（1年） | $10-15 |
| DeepSeek API 预充值 | $0（免费 500 万 tokens） |
| 智谱Z.ai | $18（Coding Plan Lite 月费） |
| 小米MiMo | ¥20 ≈ $3（注册赠送） |
...
| 决策 | 选择 | 理由 |
|------|------|------|
| API 网关 | OneAPI（MIT） | 开源成熟，OpenAI 兼容，社区活跃 |
| 部署方式 | Docker Compose 单机 | 单人运维，简单可靠 |
| 数据库 | SQLite（WAL 模式） | 轻量，零运维，够用 |
| 前端框架 | Next.js + Tailwind + shadcn/ui | 现代国际风，快速开发 |
| 设计系统 | Together AI DESIGN.md | AI 基础设施平台风格，国际现代 |
| 支付（美元） | Stripe Checkout | 国际通用，文档完善 |
| 支付（代币） | USDT TRC-20 | 低手续费，覆盖受限地区 |
| 反向代理 | Nginx + Let's Encrypt | 成熟稳定，自动续签 |
| 监控告警 | Prometheus + Telegram Bot | 轻量，适合单人 |
| 配置管理 | YAML + Git | 配置即代码，可审计回滚 |

### 附录 B: 文档索引

| 文档 | 说明 |
|------|------|
| `tech-execution-report-v5.md` | v5 完整技术报告（2656行，5大模块深度方案） |
| `中国AI模型厂商API接入调研报告.md` | v6 智谱/字节/月暗深度调研 |
| `中国AI模型厂商调研_账号池管理策略.md` | v6 小米/零一 + 账号池策略 |
| `china-ai-api-provider-research.md` | v6 英文版调研 |
| `AI-Model-Aggregation-Platform-Report-v8.md` | v8 参考（含前端UI设计原型） |

> 共享目录：`E:\ProgramData\deer-flow\projects\mxjhpt\`（桥-华双方可读写）

---

*本报告为 AI 模型 API 聚合平台的完整技术执行方案，后续所有子任务拆分、编码执行、验收交付均以此文档为权威基准。*
