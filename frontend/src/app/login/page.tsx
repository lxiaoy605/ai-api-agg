...
export default function LoginPage() {
  ...
  // Handle OAuth callback: auto-login with token
  useEffect(() => {
    if (oauthToken && !user) {
      localStorage.setItem("ai_api_agg_token", oauthToken);
      document.cookie = `ai_api_agg_token=${encodeURIComponent(oauthToken)}; path=/; max-age=86400; SameSite=Lax`;
      router.replace("/dashboard");
    }
  }, [oauthToken, user, router]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Only re-render error if it changed
      const msg =
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "var(--page-bg)" }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[var(--body-text)]">{t("auth.signInTitle")}</h1>
        </div>

        <div className="rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)] p-8 shadow-lg">
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Error banner — only shows when error !== null */}
            <div
            ...
            <div>
              ...
              <input
                ...
                autoFocus
                disabled={submitting}
                className="w-full px-3 py-2.5 bg-[var(--surface-raised)] border border-[var(--border-color)] rounded-lg text-[var(--body-text)] text-sm placeholder-[var(--muted-text)] focus:outline-none focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/20 transition-colors disabled:opacity-50"
              />
            </div>
            ...
            <div className="flex justify-end">
              <Link
                ...
                className="text-xs text-[var(--muted-text)] hover:text-brand-400 transition-colors"
              >
